...
MODEL_DIR = 'train_dir'
MODEL_NAME = 'vul_model.h5'
LOADED_DATA_DIR = 'loaded_dataset'

TEST_SIZE = 5000
ADVERSARIAL_DIR = 'adv_imgs'
BATCH_SIZE = 32
...
if __name__ == '__main__':
    ...
    # -------------------------------------------------------------------------------------- #
    # print('# ---------------- ACCURACY USING CUSTOM CODE ------------------------------- #')
    # -------------------------------------------------------------------------------------- #
    # shape = (TEST_SIZE,) + y_test.shape[1:]
    # dType = y_test.dtype
    # real_labels = generate_adversarial_data(sess=sess, x=x, y=y, predictions=preds, shape=shape, dType=dType,
    #                                         features=x_test, labels=y_test, batch_size=BATCH_SIZE)
    # acc = np.mean(
    #     (np.argmax(y_test, axis=-1)) == (np.argmax(real_labels, axis=-1))
    # )
    # print('Test accuracy on legitimate examples: ' + str(acc * 100) + '%')
    # -------------------------------------------------------------------------------------- #
    # print('# ---------------- ACCURACY USING CUSTOM CODE END ---------------------------- #')
    # -------------------------------------------------------------------------------------- #

    print('Initializing the Fast Gradient Sign Method (FGSM) attack object and graph')
    # todo: session is none in facenet
    ...
    # adv_x = tf.stop_gradient(adv_x) # this step does not occur in facenet | findout diff between identity and stopgradient
    ...
    #                       SIZE, os.path.join(ADVERSARIAL_DIR, 'noise'))
    # save_images_to_folder(
    #     x_test, SIZE, os.path.join(ADVERSARIAL_DIR, 'regular'))
